<x-admin-layout :title="'Planning — ' . $opticien->name">

@php
    $joursMap = [1 => 'Lundi', 2 => 'Mardi', 3 => 'Mercredi', 4 => 'Jeudi', 5 => 'Vendredi', 6 => 'Samedi', 7 => 'Dimanche'];
    $baseUrl = url("dashboard/opticiens/{$opticien->id}/planning");
@endphp
...
<div x-data="{
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-lg font-semibold text-gray-900">Planning — {{ $opticien->name }}</h2>
                <p class="text-sm text-gray-500">{{ $cabinet->nom }}</p>
            </div>
        </div>
    ...
    @if($creneaux->isEmpty())
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
            ...
            <p class="text-sm text-gray-400 mt-1">Définissez les disponibilités de cet opticien.</p>
        </div>
    @else
            ...
            @foreach($creneaux as $creneau)
                <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 {{ !$creneau->est_actif ? 'opacity-60' : '' }}">
                    ...
                    <div class="text-xs text-gray-500 space-y-0.5 mb-3">
                        <p>Durée : {{ $creneau->duree_consultation }} min</p>
                    ...
                    <div class="flex gap-2">
                        ...
                        <button @click="setDelete({{ $creneau->id }})"
                                class="text-xs py-1 px-2 border border-red-200 text-red-400 rounded-lg hover:bg-red-50 transition">×</button>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <x-modal name="create-creneau" max-width="md" :show="$errors->isNotEmpty() && old('_modal') === 'create-creneau'">
        <form method="POST" action="{{ route('admin.opticiens.planning.store', $opticien) }}" class="p-6" novalidate>
            @csrf
            ...
            <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach([1=>'Lun',2=>'Mar',3=>'Mer',4=>'Jeu',5=>'Ven',6=>'Sam',7=>'Dim'] as $n => $j)
                            <label class="cursor-pointer">
                                <input type="checkbox" name="jours_semaine[]" value="{{ $n }}" class="sr-only peer"/>
                ...
                <div>
                    ...
                    <x-text-input id="cr_prix" name="prix" type="number" min="0" step="500" class="mt-1 block w-full" :value="old('prix')"/>
                ...
                <div class="col-span-2">
                    <label class="flex items-start gap-3 p-3 rounded-xl border border-gray-200 hover:bg-gray-50 cursor-pointer transition">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-0.5">Les patients pourront demander une téléconsultation.</p>
                        </div>
                    </label>
                </div>
            </div>
    ...
    <x-modal name="edit-creneau" max-width="md" :show="$errors->isNotEmpty() && old('_modal') === 'edit-creneau'">
        <form method="POST" :action="`{{ $baseUrl }}/${editItem?.id}`" class="p-6">
            @csrf
            @method('PUT')
    ...
    <x-modal name="delete-creneau" max-width="sm">
        <div class="p-6">
            ...
            <form method="POST" :action="`{{ $baseUrl }}/${deleteId}`">
                @csrf
                @method('DELETE')
