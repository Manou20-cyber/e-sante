<x-admin-layout title="Conversation — {{ $interlocuteur->name }}">
...
<div class="flex items-center gap-4 mb-4">
    ...
    <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full bg-teal-100 flex items-center justify-center font-bold text-teal-700 text-lg">
            {{ strtoupper(substr($interlocuteur->name, 0, 1)) }}
        </div>
        <div>
            <p class="font-semibold text-gray-900 text-sm">{{ $interlocuteur->name }}</p>
            <p class="text-xs text-gray-500">{{ $interlocuteur->email }}</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col"
        ...
        @else
            @foreach($thread as $message)
                ...
                <div class="flex {{ $isMine ? 'justify-end' : 'justify-start' }} items-end gap-2">
                    ...
                    @if(!$isMine)
                        <div class="w-7 h-7 rounded-full bg-teal-100 flex items-center justify-center text-xs font-bold text-teal-700 shrink-0 mb-1">
    ...
    <div class="border-t border-gray-100 px-4 py-3 bg-gray-50 rounded-b-2xl">
        <form method="POST"
              action="{{ route('admin.messages.repondre', $interlocuteur) }}"
            ...
            <p class="text-xs text-gray-400 mt-1.5">
                <kbd class="px-1 py-0.5 bg-gray-200 rounded text-xs">Entrée</kbd> pour envoyer ·
