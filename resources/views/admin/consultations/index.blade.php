...
<div x-data="{
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($consultations as $consultation)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3 text-gray-900 font-medium">
                                {{ $consultation->montant ? number_format($consultation->montant, 2).' €' : '-' }}
                            </td>
                            <td class="px-6 py-3">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-gray-400">Aucune consultation</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <x-modal name="create-consultation" max-width="lg">
        <form method="POST" action="{{ route('admin.consultations.store') }}" class="p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <x-input-label for="cs_montant" value="Montant (€)"/>
    ...
    <x-modal name="edit-consultation" max-width="lg">
        <form method="POST" :action="`{{ url('admin/consultations') }}/${editItem?.id}`" class="p-6">
            @csrf
            @method('PUT')
    ...
    <x-modal name="delete-consultation" max-width="sm">
        <div class="p-6">
            ...
            <form method="POST" :action="`{{ url('admin/consultations') }}/${deleteId}`">
                @csrf
                @method('DELETE')
